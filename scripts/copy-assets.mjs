...
for (const file of await readdir('node_modules/tesseract.js-core')) {
 if (/\.wasm(\.js)?$/.test(file)) await copyFile('node_modules/tesseract.js-core/'+file,new URL(file,dest));
}
await copyFile('node_modules/@tesseract.js-data/eng/4.0.0_best_int/eng.traineddata.gz',new URL('eng.traineddata.gz',dest));
